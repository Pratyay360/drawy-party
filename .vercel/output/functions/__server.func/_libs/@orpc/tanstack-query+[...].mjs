...
import { C as get, D as isTypescriptObject, F as stringifyJSON, I as toArray, T as isAsyncIteratorObject, d as RECURSIVE_CLIENT_UNWRAP_KEYS } from "./client+[...].mjs";
...
function experimental_serializableStreamedQuery(queryFn, { refetchMode = "reset", maxChunks = Number.POSITIVE_INFINITY } = {}) {
	return async (context) => {
		...
		if (hasPreviousData) {
			if (refetchMode === "reset") query.setState({
				status: "pending",
				data: void 0,
				error: null,
...
function createRouterUtils(client, options = {}) {
	...
	return new Proxy({
		...
		if (typeof prop !== "string" || RECURSIVE_CLIENT_UNWRAP_KEYS.has(prop) || !isTypescriptObject(nextClient)) return value;
		...
		return new Proxy(value, { get(target2, prop2) {
			if (typeof prop2 !== "string" || RECURSIVE_CLIENT_UNWRAP_KEYS.has(prop2)) return Reflect.get(target2, prop2);
			return Reflect.get(nextUtils, prop2);
		} });
	} });
}
//#endregion
export { skipToken as _, ensureQueryFn as a, systemSetTimeoutZero as b, hashQueryKeyByOptions as c, matchMutation as d, matchQuery as f, resolveQueryValue as g, replaceData as h, addToStart as i, isServer as l, partialMatchKey as m, addConsumeAwareSignal as n, functionalUpdate as o, noop as p, addToEnd as r, hashKey as s, createRouterUtils as t, isValidTimeout as u, sleep as v, timeoutManager as x, timeUntilStale as y };
