...
function createChunkedHandler(receive: (data: Uint8Array) => void) {
    ...
    return (message: { data: unknown }) => {
                ...
                try {
                    ...
                    if (marker.type === "start") {
                        ...
                        if (
                            ...
                            console.warn("[drawy] chunked batch mismatch", { start, marker, batchLen: batch.length, size });
                        }
                        batch = undefined;
                        start = undefined;
                    }
        ...
        } else if (message.data instanceof ArrayBuffer || message.data instanceof Uint8Array || ArrayBuffer.isView(message.data)) {
...
export default class EditorServer implements Party.Server {
    ...
    async onConnect(conn: Party.Connection) {
        ...
        (conn as unknown as { addEventListener: (t: string, h: (e: unknown) => void) => void }).addEventListener("message", (evt: unknown) => {
            const data = (evt as { data?: unknown })?.data;
            // Only handle binary/Uint8Array here; string messages fall through to onMessage
            if (typeof data === "string") {
                if ((data as string).startsWith(BATCH_SENTINEL)) handleYjsBinary({ data } as { data: unknown });
                return;
            }
    ...
    private handleYjsMessage(conn: Party.Connection, bytes: Uint8Array) {
        ...
        switch (messageType) {
            case messageSync: {
                ...
                if (msgType === syncProtocol.messageYjsUpdate || msgType === syncProtocol.messageYjsSyncStep2) {
            ...
            case messageAwareness: {
                ...
                try {
                    ...
                    if (e instanceof Error && e.message.includes("Unexpected end of array")) {
    ...
    onMessage(message: string | ArrayBuffer, sender: Party.Connection) {
        ...
        if (data["type"] === "scene" || data["type"] === "saved" || data["event"] === "realtime-cursor-move") {
